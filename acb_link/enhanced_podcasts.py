...
from .podcast_manager import PodcastManager, Podcast, PodcastEpisode, DownloadManager
...
class EnhancedPodcastsPanel(wx.Panel):
    ...
    def _build_ui(self):
        ...
        self.splitter = wx.SplitterWindow(
            self, 
            style=wx.SP_3D | wx.SP_LIVE_UPDATE | wx.SP_NOBORDER
        )
    ...
    def _create_episodes_panel(self, parent: wx.Window) -> wx.Panel:
        ...
        self.sort_choice = wx.Choice(
            panel,
            choices=[SORT_LABELS[s] for s in EpisodeSortOrder]
        )
    ...
    def _sort_episodes(
        ...
        if sort_order == EpisodeSortOrder.DATE_NEWEST:
            ...
            return sorted(episodes, key=lambda e: (not e.downloaded, e.pub_date or ""), reverse=True)
    ...
    def _on_download_episode(self, event):
        ...
        if episode.downloaded:
            ...
            if result == wx.YES:
                self.podcast_manager.delete_download(
                    self.current_podcast.id, 
                    episode.id
                )
                self._sort_and_display_episodes()
    ...
    def _on_mark_played(self, event):
        ...
        if episode.played:
            ...
            self.podcast_manager.mark_episode_played(
                self.current_podcast.id,
                episode.id
            )
            self.mark_played_btn.SetLabel("↩ Mark Unplayed")
...
"""
        ...
        dlg = wx.MessageDialog(
            self,
            details,
            f"Episode Details - {episode.title[:50]}",
