...
class ViewSettingsManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "view_settings.json"
        )
...
class PaneNavigator:
    ...
    def register_pane(
        ...
        self.panes.append(NavigablePane(
            pane_type=pane_type,
            window=window,
            label=label,
...
class LowVisionSettings:
    ...
    color_scheme: str = "system"  # "system", "light", "dark", "high_contrast_light", "high_contrast_dark"
...
class VisualSettingsManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "visual_settings.json"
        )
        self.settings = self._load()
...
class ViewSettingsDialog(wx.Dialog):
    ...
    def _create_appearance_tab(self, parent: wx.Window) -> wx.Panel:
        ...
        scheme_sizer.Add(wx.StaticText(panel, label="Color scheme:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10)
        ...
        font_sizer.Add(wx.StaticText(panel, label="Font size:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10)
        
        self.font_spin = wx.SpinCtrl(panel, min=8, max=32, initial=self.visual_manager.settings.font_size)
    ...
    def _create_accessibility_tab(self, parent: wx.Window) -> wx.Panel:
        ...
        self.focus_width_spin = wx.SpinCtrl(
            panel, min=1, max=10, 
            initial=self.visual_manager.settings.focus_indicator_width
        )
...
class SchedulePreviewDialog(wx.Dialog):
    ...
    def __init__(
        self,
        parent: wx.Window,
        schedule_manager: StreamScheduleManager,
        stream_name: str
    ):
        super().__init__(
            parent,
