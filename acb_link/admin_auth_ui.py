...
class AdminLoginDialog(wx.Dialog):
    ...
    def __init__(self, parent: Optional[wx.Window] = None,
        ...
        announce("Administrator login dialog. Enter your GitHub Personal Access Token to access admin features.")
    ...
    def _build_ui(self):
        ...
        scope_info = wx.StaticText(
            panel,
            label="Required token scopes: read:org (to check membership)"
        )
    ...
    def _update_display(self):
        ...
        if session and session.role != AdminRole.USER:
            self.session_label.SetLabel(
                f"Logged in: {session.display_name} ({session.role.name})"
            )
...
def show_admin_login_dialog(parent: Optional[wx.Window] = None) -> Tuple[bool, Optional[AdminSession]]:
    """
