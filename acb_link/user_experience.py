...
class ListeningStats:
    ...
    def record_session(self, content_type: str, content_name: str, duration_seconds: int):
        ...
        if content_type == "stream":
            ...
            # Update favorite
            if not self.favorite_stream or self.streams_listened.get(
                content_name, 0
            ) > self.streams_listened.get(self.favorite_stream, 0):
            ...
            if not self.favorite_podcast or self.podcasts_listened.get(
                content_name, 0
            ) > self.podcasts_listened.get(self.favorite_podcast, 0):
...
class ListeningStatsManager:
    ...
    def __init__(self, stats_path: Optional[str] = None):
        self.stats_path = stats_path or str(Path.home() / ".acb_link" / "listening_stats.json")
        ...
        if self._current_session_start:
            ...
            if duration >= 30:  # Only count sessions > 30 seconds
                self.stats.record_session(
                    self._current_content_type, self._current_content_name, duration
                )
                self.save()
...
class ResumePlaybackManager:
    ...
    def __init__(self, state_path: Optional[str] = None):
        self.state_path = state_path or str(Path.home() / ".acb_link" / "last_playback.json")
...
class PanicKeyHandler:
    ...
    def __init__(
        self, mute_callback: Callable, minimize_callback: Callable, is_muted_callback: Callable
    ):
...
class WelcomeWizard(wx.Dialog):
    ...
    def __init__(self, parent: Optional[wx.Window] = None, settings=None):
        super().__init__(parent, title="Welcome to ACB Link", style=wx.DEFAULT_DIALOG_STYLE)
    ...
    def _create_theme_page(self, parent) -> wx.Panel:
        ...
        self.theme_radio = wx.RadioBox(
            page, choices=[t[0] for t in themes], majorDimension=1, style=wx.RA_SPECIFY_COLS
        )
        ...
        self.font_slider = wx.Slider(
            page, value=12, minValue=8, maxValue=24, style=wx.SL_HORIZONTAL | wx.SL_LABELS
        )
    ...
    def _create_accessibility_page(self, parent) -> wx.Panel:
        ...
        self.chk_announcements = wx.CheckBox(page, label="&Enable screen reader announcements")
        ...
        self.chk_audio_feedback = wx.CheckBox(page, label="Play &audio feedback sounds")
...
class ListeningStatsDialog(wx.Dialog):
    ...
    def __init__(self, parent: wx.Window, stats: ListeningStats):
        super().__init__(
            parent, title="Listening Statistics", style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
        )
    ...
    def _create_ui(self):
        ...
        if self.stats.streams_listened:
            ...
            sorted_streams = sorted(
                self.stats.streams_listened.items(), key=lambda x: x[1], reverse=True
            )[:5]
...
class ExportImportDialog(wx.Dialog):
    ...
    def __init__(self, parent: wx.Window, backup_manager: SettingsBackupManager):
        super().__init__(parent, title="Export / Import Settings", style=wx.DEFAULT_DIALOG_STYLE)
    ...
    def _create_ui(self):
        ...
        export_desc = wx.StaticText(panel, label="Create a backup of all your settings and data.")
        ...
        import_desc = wx.StaticText(panel, label="Restore settings from a backup file.")
            ...
            if dialog.ShowModal() == wx.ID_OK:
                ...
                if wx.MessageBox(msg, "Confirm Import", wx.YES_NO | wx.ICON_QUESTION) == wx.YES:
                    ...
                    if success:
                        ...
                        wx.MessageBox(
                            f"Import failed: {message}", "Import Error", wx.OK | wx.ICON_ERROR
                        )
...
class ResumePlaybackDialog(wx.Dialog):
    ...
    def __init__(self, parent: wx.Window, resume_manager: ResumePlaybackManager):
        super().__init__(parent, title="Resume Playback", style=wx.DEFAULT_DIALOG_STYLE)
