...
def detect_distribution_channel() -> DistributionChannel:
    ...
    if system == "windows":
        ...
        try:
            ...
            key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Software\ACB\ACB Link Desktop")
            winreg.CloseKey(key)
    ...
    def generate_manifest(self) -> Dict[str, FileManifest]:
        ...
        for file_path in self.install_path.rglob("*"):
            if file_path.is_file():
                # Skip certain files
                if any(skip in str(file_path) for skip in ["__pycache__", ".pyc", ".log", "cache"]):
                    continue
    ...
    def check_for_delta_update(
        ...
        if callback:
            thread = threading.Thread(
                target=self._check_delta_async, args=(target_version, callback), daemon=True
            )
            thread.start()
            return None
        else:
    ...
    def _do_check_delta(self, target_version: str) -> Optional[DeltaUpdateInfo]:
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                logger.info(
                    f"No delta update available from {self.current_version} to {target_version}"
                )
                return None
...
class MicrosoftStoreUpdater:
    ...
    def check_for_update() -> bool:
        ...
        try:
            ...
            subprocess.run(["start", "ms-windows-store://updates"], shell=True, check=False)
            return True
        except Exception as e:
...
class MacAppStoreUpdater:
    ...
    def check_for_update() -> bool:
        ...
        try:
            ...
            subprocess.run(["open", "macappstore://showUpdatesPage"], check=False)
            return True
        except Exception as e:
...
class DistributionManager:
    ...
    def check_for_updates(self, callback: Optional[Callable[[bool, str], None]] = None):
        """
