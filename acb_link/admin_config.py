...
class OrganizationConfig:
    ...
    org_defaults: Dict[str, Any] = field(
        default_factory=lambda: {
            "default_volume": 80,
...
class ConfigSigner:
    ...
    def verify_signature(cls, signed_config: SignedConfig) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Verify signature
            expected = cls._compute_signature(signed_config.config, cls.VERIFICATION_KEY)
    ...
    @classmethod
    def sign_config(cls, config: Dict[str, Any], server_id: str, signing_key: str) -> SignedConfig:
        """
...
class AdminConfigManager:
    ...
    def is_admin(self) -> bool:
        ...
        return (
            ...
            and self._current_token.role != AdminRole.USER
        )
    ...
    async def sync_config_from_server(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            server_url = (
                self._org_config.config_server if self._org_config else DEFAULT_CONFIG_SERVER
            )
    ...
    def _sync_config_sync(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Create request
            req = urllib.request.Request(url, headers={"User-Agent": "ACBLinkDesktop/1.0"})
    ...
    def _authenticate_admin_sync(self, username: str, password: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={"Content-Type": "application/json", "User-Agent": "ACBLinkDesktop/1.0"},
                method="POST",
            )
    ...
    async def push_config_to_server(
        self, changes: Dict[str, Any], section: ConfigSection
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    _data = await response.json()  # noqa: F841 - response consumed

            # Audit log
            self._audit_log(
                "config_update", self._current_token.username, section.value, {"changes": changes}
            )
