...
class WakeWordDetector:
    ...
    def start(self):
        ...
        if self._use_openwakeword:
            ...
            self._listen_thread = threading.Thread(
                target=self._wake_listen_loop, daemon=True, name="WakeWordDetector-SpeechRec"
            )
        else:
            self._is_listening = False
            return
        ...
        try:
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    # Convert bytes to int16 array using struct (avoids numpy dependency)
                    # Each sample is 2 bytes (16-bit signed integer, little-endian)
                    sample_count = len(audio_data) // 2
                    audio_array = array.array("h", struct.unpack(f"<{sample_count}h", audio_data))
...
class NaturalLanguageProcessor:
    ...
    def process(self, text: str) -> NaturalLanguageQuery:
        ...
        return NaturalLanguageQuery(
            intent="unknown", target="", parameters={}, original_text=text, confidence=0.3
        )

    def _extract_parameters(self, intent: str, target: str, text: str) -> Dict[str, Any]:
...
class EnhancedVoiceController:
    ...
    def _start_command_listening(self):
        ...
        self._command_thread = threading.Thread(
            target=self._listen_for_command, daemon=True, name="CommandListener"
        )
    ...
    def _listen_for_command(self):
        ...
        try:
            with sr.Microphone() as source:
                ...
                # Listen for command
                audio = self.recognizer.listen(  # type: ignore[union-attr]
                    source, timeout=self._command_timeout, phrase_time_limit=10.0
                )
...
class VoiceGuidedSetup:
    ...
    def _setup_default_tab(self):
        ...
        self.voice.speak(
            "Which tab would you like to see when the app opens? " "Say home, streams, or podcasts."
        )

    def _setup_notifications(self):
        """Ask about notifications."""
        self.voice.speak(
            "Would you like to receive notifications about live events? " "Say yes or no."
        )
