...
from .data import RECORDING_BITRATES, SPEED_OPTIONS, SPEED_VALUES, TAB_LABELS, TAB_NAMES
...
class SettingsDialog(wx.Dialog):
    ...
    def _build_ui(self):
        ...
        make_accessible(
            self.notebook, "Settings categories", "Use arrow keys to navigate between settings tabs"
        )
    ...
    def _add_general_tab(self):
        ...
        tab_row.Add(
            wx.StaticText(panel, label="Default tab on startup:"),
            0,
    ...
    def _add_appearance_tab(self):
        ...
        theme_row.Add(
            wx.StaticText(panel, label="Color theme:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.theme_choice = wx.Choice(
            panel,
            choices=[
                "System Default",
                "Light",
        ...
        family_row.Add(
            wx.StaticText(panel, label="Font family:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        ...
        focus_row.Add(
            wx.StaticText(panel, label="Focus ring width:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.focus_width = wx.SpinCtrl(
            panel, value=str(self.settings.theme.focus_ring_width), min=1, max=5
        )
    ...
    def _add_playback_tab(self):
        ...
        back_row.Add(
            wx.StaticText(panel, label="Skip backward:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.skip_backward = wx.SpinCtrl(
            panel, value=str(self.settings.playback.skip_backward_seconds), min=5, max=120
        )
        ...
        fwd_row.Add(
            wx.StaticText(panel, label="Skip forward:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.skip_forward = wx.SpinCtrl(
            panel, value=str(self.settings.playback.skip_forward_seconds), min=5, max=120
        )
        ...
        speed_row.Add(
            wx.StaticText(panel, label="Default speed:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.playback_speed = wx.Choice(panel, choices=SPEED_OPTIONS)
        if self.settings.playback.default_speed in SPEED_VALUES:
            self.playback_speed.SetSelection(
                SPEED_VALUES.index(self.settings.playback.default_speed)
            )
        else:
        ...
        vol_row.Add(
            wx.StaticText(panel, label="Default volume:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.default_volume = wx.Slider(
            panel,
        ...
        self.default_volume.Bind(
            wx.EVT_SLIDER, lambda e: self.vol_label.SetLabel(f"{self.default_volume.GetValue()}%")
        )
        ...
        preset_row.Add(
            wx.StaticText(panel, label="Preset:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        ...
        self.eq_bass = wx.Slider(
            panel,
            value=self.settings.playback.eq_bass + 10,
            minValue=0,
            maxValue=20,
            size=(150, -1),
        )
        ...
        self.eq_mid = wx.Slider(
            panel, value=self.settings.playback.eq_mid + 10, minValue=0, maxValue=20, size=(150, -1)
        )
        ...
        self.eq_treble = wx.Slider(
            panel,
            value=self.settings.playback.eq_treble + 10,
            minValue=0,
            maxValue=20,
            size=(150, -1),
        )
        ...
        self.eq_bass.Bind(
            wx.EVT_SLIDER, lambda e: self.bass_label.SetLabel(f"{self.eq_bass.GetValue() - 10:+d}")
        )
        self.eq_mid.Bind(
            wx.EVT_SLIDER, lambda e: self.mid_label.SetLabel(f"{self.eq_mid.GetValue() - 10:+d}")
        )
        self.eq_treble.Bind(
            wx.EVT_SLIDER,
            lambda e: self.treble_label.SetLabel(f"{self.eq_treble.GetValue() - 10:+d}"),
        )
    ...
    def _add_storage_tab(self):
        ...
        path_row.Add(
            wx.StaticText(panel, label="Download folder:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.podcast_path = wx.TextCtrl(
            panel, value=self.settings.storage.podcast_download_path, size=(350, -1)
        )
        ...
        rec_path_row.Add(
            wx.StaticText(panel, label="Recording folder:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.recording_path = wx.TextCtrl(
            panel, value=self.settings.storage.recording_path, size=(350, -1)
        )
        ...
        format_row.Add(
            wx.StaticText(panel, label="Recording format:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        ...
        self.recording_format.SetSelection(
            format_map.get(self.settings.storage.recording_format, 0)
        )
        format_row.Add(self.recording_format, 0, wx.RIGHT, 20)

        format_row.Add(
            wx.StaticText(panel, label="Bitrate:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.recording_bitrate = wx.Choice(
            panel, choices=["64 kbps", "96 kbps", "128 kbps", "192 kbps", "256 kbps", "320 kbps"]
        )
        bitrate_map = {64: 0, 96: 1, 128: 2, 192: 3, 256: 4, 320: 5}
        self.recording_bitrate.SetSelection(
            bitrate_map.get(self.settings.storage.recording_bitrate, 2)
        )
        ...
        cache_row.Add(
            wx.StaticText(panel, label="Maximum cache size:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.cache_size = wx.SpinCtrl(
            panel, value=str(self.settings.storage.max_cache_size_mb), min=100, max=5000
        )
    ...
    def _add_home_tab(self):
        ...
        max_row.Add(
            wx.StaticText(panel, label="Max podcasts to show:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.max_podcasts = wx.SpinCtrl(
            panel, value=str(self.settings.home_tab.max_podcasts), min=1, max=20
        )
        ...
        recent_row.Add(
            wx.StaticText(panel, label="Max recent items:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.max_recent = wx.SpinCtrl(
            panel, value=str(self.settings.home_tab.max_recent), min=1, max=50
        )
    ...
    def _add_startup_tab(self):
        ...
        self.chk_startup_to_tray = wx.CheckBox(
            panel, label="Start minimized to system tray (completely hidden)"
        )
        self.chk_startup_to_tray.SetValue(self.settings.startup.start_minimized_to_tray)
        self.chk_startup_to_tray.SetToolTip(
            "Start hidden in the system tray - no window or taskbar button"
        )
        ...
        self.chk_auto_play_startup = wx.CheckBox(
            panel, label="Automatically resume playback on startup"
        )
        ...
        delay_row.Add(
            wx.StaticText(panel, label="Auto-play delay:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.auto_play_delay = wx.SpinCtrl(
            panel, value=str(self.settings.startup.auto_play_delay_seconds), min=0, max=30
        )
        ...
        self.chk_sync_data_startup = wx.CheckBox(
            panel, label="Sync data from ACB servers on startup"
        )
    ...
    def _add_system_tray_tab(self):
        ...
        self.chk_minimize_tray = wx.CheckBox(
            panel, label="Minimize to system tray (hide from taskbar)"
        )
        ...
        dbl_row.Add(
            wx.StaticText(panel, label="Double-click action:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        ...
        self.tray_dbl_click.SetSelection(
            action_map.get(self.settings.system_tray.double_click_action, 0)
        )
        ...
        single_row.Add(
            wx.StaticText(panel, label="Single-click action:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.tray_single_click = wx.Choice(
            panel, choices=["None", "Show/Hide window", "Play/Pause", "Show menu"]
        )
        single_map = {"none": 0, "show_hide": 1, "play_pause": 2, "show_menu": 3}
        self.tray_single_click.SetSelection(
            single_map.get(self.settings.system_tray.single_click_action, 0)
        )
        ...
        self.chk_now_playing_notif.SetValue(
            self.settings.system_tray.show_now_playing_notifications
        )
        notif_sizer.Add(self.chk_now_playing_notif, 0, wx.ALL, 5)

        dur_row = wx.BoxSizer(wx.HORIZONTAL)
        dur_row.Add(
            wx.StaticText(panel, label="Notification duration:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.notif_duration = wx.SpinCtrl(
            panel, value=str(self.settings.system_tray.notification_duration), min=1, max=30
        )
    ...
    def _add_notifications_tab(self):
        ...
        dur_row = wx.BoxSizer(wx.HORIZONTAL)
        dur_row.Add(
            wx.StaticText(panel, label="Display duration:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.notif_display_duration = wx.SpinCtrl(
            panel, value=str(self.settings.notifications.duration_seconds), min=1, max=30
        )
        ...
        pos_row.Add(
            wx.StaticText(panel, label="Position:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.notif_position = wx.Choice(
            panel, choices=["Bottom Right", "Bottom Left", "Top Right", "Top Left"]
        )
        ...
        self.chk_notif_now_playing = wx.CheckBox(
            panel, label="Now playing (stream/episode changes)"
        )
        ...
        self.chk_notif_stream_changes = wx.CheckBox(
            panel, label="Stream status changes (connected/disconnected)"
        )
    ...
    def _add_keyboard_tab(self):
        ...
        self.chk_global_hotkeys = wx.CheckBox(
            panel, label="Enable global hotkeys (work when app is in background)"
        )
        ...
        self.chk_media_keys = wx.CheckBox(
            panel, label="Respond to media keys (play/pause, stop, etc.)"
        )
        ...
        self.chk_vim_nav = wx.CheckBox(
            panel, label="Vim-style navigation (h/j/k/l for left/down/up/right)"
        )
    ...
    def _add_voice_control_tab(self):
        ...
        self.chk_wake_word_enabled.SetToolTip(
            "If enabled, say the wake word first to activate listening"
        )
        wake_sizer.Add(self.chk_wake_word_enabled, 0, wx.ALL, 5)

        wake_row = wx.BoxSizer(wx.HORIZONTAL)
        wake_row.Add(
            wx.StaticText(panel, label="Wake word/phrase:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.txt_wake_word = wx.TextCtrl(panel, value=self.settings.voice.wake_word, size=(200, -1))
        self.txt_wake_word.SetToolTip(
            "The phrase that activates voice control (e.g., 'hey link', 'ok computer')"
        )
        make_accessible(
            self.txt_wake_word, "Wake word", "Enter the phrase to activate voice control"
        )
        wake_row.Add(self.txt_wake_word, 0)
        wake_sizer.Add(wake_row, 0, wx.ALL, 5)

        wake_note = wx.StaticText(
            panel, label="Examples: 'hey link', 'ok link', 'computer', 'hey radio'"
        )
        ...
        rate_row.Add(
            wx.StaticText(panel, label="Speaking rate:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.spin_tts_rate = wx.SpinCtrl(
            panel, value=str(self.settings.voice.tts_rate), min=100, max=300
        )
        ...
        rate_row.Add(
            wx.StaticText(panel, label=" words/minute"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 5
        )
        tts_sizer.Add(rate_row, 0, wx.ALL, 5)

        vol_row = wx.BoxSizer(wx.HORIZONTAL)
        vol_row.Add(
            wx.StaticText(panel, label="TTS Volume:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.slider_tts_volume = wx.Slider(
            panel,
            value=int(self.settings.voice.tts_volume * 100),
            minValue=0,
            maxValue=100,
            size=(150, -1),
        )
        ...
        self.lbl_tts_volume = wx.StaticText(
            panel, label=f"{int(self.settings.voice.tts_volume * 100)}%"
        )
    ...
    def _add_announcements_tab(self):
        ...
        try:
            ...
            self.announcement_settings_panel = AnnouncementSettingsPanel(panel, manager.settings)
    ...
    def _add_privacy_tab(self):
        ...
        max_row = wx.BoxSizer(wx.HORIZONTAL)
        max_row.Add(
            wx.StaticText(panel, label="Maximum history items:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.max_history = wx.SpinCtrl(
            panel, value=str(self.settings.privacy.max_history_items), min=10, max=1000
        )
        ...
        note = wx.StaticText(
            panel, label="All analytics are disabled by default and require your explicit consent."
        )
        note.SetForegroundColour(wx.Colour(100, 100, 100))
        analytics_sizer.Add(note, 0, wx.ALL, 5)

        self.chk_analytics = wx.CheckBox(
            panel, label="Help improve ACB Link by sharing anonymous usage data"
        )
    ...
    def _add_performance_tab(self):
        ...
        dl_row.Add(
            wx.StaticText(panel, label="Maximum concurrent downloads:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.max_downloads = wx.SpinCtrl(
            panel, value=str(self.settings.performance.max_concurrent_downloads), min=1, max=10
        )
        ...
        log_row.Add(
            wx.StaticText(panel, label="Maximum log file size:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.max_log_size = wx.SpinCtrl(
            panel, value=str(self.settings.performance.max_log_size_mb), min=1, max=100
        )
        ...
        self.chk_auto_cleanup_logs = wx.CheckBox(
            panel, label="Automatically clean up old log files"
        )
    ...
    def _add_accessibility_tab(self):
        ...
        self.chk_kb_only = wx.CheckBox(
            panel, label="Keyboard-only mode (disable mouse interactions)"
        )
    ...
    def _save_settings(self):
        ...
        self.settings.storage.recording_bitrate = RECORDING_BITRATES[
            self.recording_bitrate.GetSelection()
        ]
        ...
        self.settings.system_tray.show_now_playing_notifications = (
            self.chk_now_playing_notif.GetValue()
        )
        self.settings.system_tray.notification_duration = self.notif_duration.GetValue()
        dbl_actions = ["show_hide", "play_pause"]
        self.settings.system_tray.double_click_action = dbl_actions[
            self.tray_dbl_click.GetSelection()
        ]
        single_actions = ["none", "show_hide", "play_pause", "show_menu"]
        self.settings.system_tray.single_click_action = single_actions[
            self.tray_single_click.GetSelection()
        ]
...
class VoiceCommandMappingDialog(wx.Dialog):
    ...
    def __init__(self, parent, settings: AppSettings):
        ...
        self.custom_triggers = (
            dict(settings.voice.custom_triggers) if settings.voice.custom_triggers else {}
        )
    ...
    def _build_ui(self):
        ...
        self.command_list = wx.ListCtrl(
            left_panel, style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.BORDER_SUNKEN, size=(250, 350)
        )
        ...
        self.lbl_selected_command = wx.StaticText(
            right_panel, label="Select a command to edit triggers"
        )
        ...
        self.txt_triggers = wx.TextCtrl(right_panel, style=wx.TE_MULTILINE, size=(350, 150))
        self.txt_triggers.SetToolTip(
            "Enter the phrases that will trigger this command, one per line"
        )
        make_accessible(
            self.txt_triggers,
            "Trigger phrases",
        ...
        self.txt_defaults = wx.TextCtrl(
            right_panel, style=wx.TE_MULTILINE | wx.TE_READONLY, size=(350, 100)
        )
    ...
    def _on_save_triggers(self, event):
        ...
        if not triggers:
            wx.MessageBox(
                "Please enter at least one trigger phrase.", "No Triggers", wx.OK | wx.ICON_WARNING
            )
            return
    ...
    def _on_reset_all(self, event):
        ...
        if (
            ...
            wx.MessageBox(
                "All triggers have been reset to defaults.", "Reset", wx.OK | wx.ICON_INFORMATION
            )
