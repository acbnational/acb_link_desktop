...
from .data import STREAMS, PODCASTS, TAB_NAMES
from .panels import HomePanel, StreamsPanel, PodcastsPanel, ResourcesPanel, AffiliatesPanel
from .dialogs import SettingsDialog
from .advanced_settings import show_advanced_settings, AdvancedSettingsWarningDialog
...
from .utils import (
    setup_logging, RecentItemsManager, PlaybackPositionManager,
    get_app_data_dir
)
from .accessibility import (
    announce, make_button_accessible, make_list_accessible,
    get_screen_reader_manager, FocusManager, LiveRegion,
...
from .search import GlobalSearch, SearchResultType
...
from .localization import get_translation_manager, _, TranslationKey
from .styles import get_style_manager, StyleManager, UIConstants, hex_to_colour

# User experience enhancements
from .user_experience import (
    ListeningStatsManager, ResumePlaybackManager, SettingsBackupManager,
...
from .playback_enhancements import (
    ...
    AutoPlayManager, AutoPlayDialog,
    PlaybackSpeedController, PlaybackSpeedDialog, PlaybackSpeedPreset,
    NowPlayingInfo, ShareManager,
    RecentlyPlayedManager, RecentlyPlayedDialog,
    MediaKeyHandler,
    AudioDuckingManager, AudioDuckingDialog,
    QuietHoursManager, QuietHoursDialog
)

# View settings and pane navigation
from .view_settings import (
    ViewSettingsManager, ViewSettingsDialog,
    PaneNavigator, PaneType, NavigablePane,
    FocusModeManager, VisualSettingsManager,
    StreamScheduleManager, FavoritesQuickDial
)
...
class MainFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        self.style_manager.set_font_size(self.settings.theme.font_size if self.settings.theme else 11)
        ...
        if (self.settings.system_tray.start_minimized or 
            self.settings.startup.start_minimized_to_tray or
            self.settings.system_tray.start_minimized_to_tray):
    ...
    def _init_managers(self):
        ...
        self.podcast_manager = PodcastManager(
            on_episode_downloaded=self._on_episode_downloaded
        )
        ...
        self.sleep_timer = SleepTimer(
            on_timer_end=self._on_sleep_timer_end
        )
        ...
        self.playlist_manager = PlaylistManager(
            on_playlists_changed=self._on_playlists_changed
        )
        self.playlist_player = PlaylistPlayer(
            playlist_manager=self.playlist_manager,
            audio_player=self.native_player
        )
        ...
        self.calendar_manager = CalendarManager(
            on_event_reminder=self._on_event_reminder
        )
        ...
        self.font_size_manager = FontSizeManager(
            self.settings.theme.font_size if self.settings.theme else 12,
        ...
        self.quick_status = QuickStatusAnnouncer(
            ...
            get_volume=lambda: self.volume_slider.GetValue() if hasattr(self, 'volume_slider') else 100,
        ...
        self.audio_ducking = AudioDuckingManager(
            get_volume=lambda: self.volume_slider.GetValue() if hasattr(self, 'volume_slider') else 100,
            set_volume=lambda v: self.volume_slider.SetValue(v) if hasattr(self, 'volume_slider') else None,
            duck_percentage=30
        )
    ...
    def _apply_wizard_settings(self, wizard_settings: dict):
        ...
        if self.settings.accessibility:
            ...
            self.settings.accessibility.audio_feedback = wizard_settings.get(
                "audio_feedback", True
            )
    ...
    def _on_connectivity_changed(self, is_online: bool):
        ...
        if not is_online:
            wx.CallAfter(self._show_notification, "Network Status", "You are now offline. Some features may be limited.")
    ...
    def _on_event_reminder(self, event):
        ...
        wx.CallAfter(
            ...
            "Event Reminder",
            f"{event.title} starts in {event.time_until}"
        )
    ...
    def _process_voice_command(self, intent: str, query):
        ...
        if intent == 'play':
            ...
            results = self.search_engine.search(query.target)
            # Navigate to search tab and show results
            self.notebook.SetSelection(5)  # Search tab index
        elif intent == 'navigate':
            target = query.target.lower()
            tab_map = {'home': 0, 'streams': 1, 'podcasts': 2, 'favorites': 3, 'playlists': 4, 'search': 5, 'calendar': 6}
    ...
    def _build_menubar(self):
        ...
        self.menu_open_browser = file_menu.Append(wx.ID_ANY, "Open in &Browser\tCtrl+B", "Open content in browser")
        file_menu.AppendSeparator()
        
        self.menu_settings = file_menu.Append(wx.ID_PREFERENCES, "&Settings...\tCtrl+,", "Open settings")
        self.menu_advanced_settings = file_menu.Append(wx.ID_ANY, "Advanced Settings...\tCtrl+Shift+,", "Open advanced configuration")
        file_menu.AppendSeparator()
        ...
        self.menu_select_all = edit_menu.Append(wx.ID_SELECTALL, "Select &All\tCtrl+A", "Select all text")
        edit_menu.AppendSeparator()
        
        self.menu_copy_now_playing = edit_menu.Append(wx.ID_ANY, "Copy What's &Playing\tCtrl+Shift+C", "Copy current track info to clipboard")
        self.menu_copy_stream_url = edit_menu.Append(wx.ID_ANY, "Copy Stream &URL", "Copy current stream URL to clipboard")
        edit_menu.AppendSeparator()
        
        # Preferences (duplicate in Edit menu for discoverability)
        self.menu_preferences = edit_menu.Append(wx.ID_ANY, "&Preferences...\tCtrl+,", "Open settings")
        ...
        self.menu_streams_tab = tabs_submenu.Append(wx.ID_ANY, "&Streams\tCtrl+2", "Go to Streams tab")
        self.menu_podcasts_tab = tabs_submenu.Append(wx.ID_ANY, "&Podcasts\tCtrl+3", "Go to Podcasts tab")
        self.menu_affiliates_tab = tabs_submenu.Append(wx.ID_ANY, "&Affiliates\tCtrl+4", "Go to Affiliates tab")
        self.menu_resources_tab = tabs_submenu.Append(wx.ID_ANY, "&Resources\tCtrl+5", "Go to Resources tab")
        view_menu.AppendSubMenu(tabs_submenu, "Go to &Tab")
        view_menu.AppendSeparator()
        
        # Show/Hide UI elements (sticky checkboxes)
        self.menu_show_toolbar = view_menu.AppendCheckItem(wx.ID_ANY, "Show &Toolbar", "Toggle toolbar visibility")
        self.menu_show_toolbar.Check(self.view_settings.settings.show_toolbar)
        
        self.menu_show_tab_bar = view_menu.AppendCheckItem(wx.ID_ANY, "Show Tab &Bar", "Toggle tab bar visibility")
        self.menu_show_tab_bar.Check(self.view_settings.settings.show_tab_bar)
        
        self.menu_show_status_bar = view_menu.AppendCheckItem(wx.ID_ANY, "Show &Status Bar", "Toggle status bar visibility")
        self.menu_show_status_bar.Check(self.view_settings.settings.show_status_bar)
        
        self.menu_show_player = view_menu.AppendCheckItem(wx.ID_ANY, "Show &Player Controls\tCtrl+Shift+P", "Toggle player controls visibility")
        self.menu_show_player.Check(self.view_settings.settings.show_player_panel)
        
        self.menu_show_now_playing = view_menu.AppendCheckItem(wx.ID_ANY, "Show &Now Playing Banner", "Toggle now playing banner")
        self.menu_show_now_playing.Check(self.view_settings.settings.show_now_playing_banner)
        
        self.menu_show_sidebar = view_menu.AppendCheckItem(wx.ID_ANY, "Show Side&bar\tCtrl+\\", "Toggle sidebar visibility")
        self.menu_show_sidebar.Check(self.view_settings.settings.show_sidebar)
        view_menu.AppendSeparator()
        
        # Pane navigation
        self.menu_next_pane = view_menu.Append(wx.ID_ANY, "&Next Pane\tF6", "Move focus to next pane")
        self.menu_prev_pane = view_menu.Append(wx.ID_ANY, "&Previous Pane\tShift+F6", "Move focus to previous pane")
        view_menu.AppendSeparator()
        
        # Focus mode
        self.menu_focus_mode = view_menu.AppendCheckItem(wx.ID_ANY, "&Focus Mode\tCtrl+Shift+D", "Minimal distraction-free interface")
        self.menu_focus_mode.Check(self.view_settings.settings.focus_mode)
        
        self.menu_full_screen = view_menu.Append(wx.ID_ANY, "F&ull Screen\tF11", "Toggle full screen mode")
        view_menu.AppendSeparator()
        
        # Quick actions
        self.menu_quick_status = view_menu.Append(wx.ID_ANY, "&Quick Status\tF5", "Announce current playback status")
        self.menu_panic_key = view_menu.Append(wx.ID_ANY, "&Panic Key (Mute && Minimize)\tCtrl+Shift+M", "Instantly mute and minimize")
        view_menu.AppendSeparator()
        
        # Text size submenu
        text_size_submenu = wx.Menu()
        self.menu_increase_font = text_size_submenu.Append(wx.ID_ANY, "&Increase\tCtrl++", "Make text larger")
        self.menu_decrease_font = text_size_submenu.Append(wx.ID_ANY, "&Decrease\tCtrl+-", "Make text smaller")
        self.menu_reset_font = text_size_submenu.Append(wx.ID_ANY, "&Reset\tCtrl+0", "Reset text to default size")
        view_menu.AppendSubMenu(text_size_submenu, "Text Si&ze")
        view_menu.AppendSeparator()
        
        # Podcast sort submenu
        self.podcast_sort_menu = wx.Menu()
        self.menu_sort_date_newest = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "&Newest First", "Sort episodes by date, newest first")
        self.menu_sort_date_oldest = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "&Oldest First", "Sort episodes by date, oldest first")
        self.podcast_sort_menu.AppendSeparator()
        self.menu_sort_title_az = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "Title &A-Z", "Sort episodes by title alphabetically")
        self.menu_sort_title_za = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "Title &Z-A", "Sort episodes by title reverse alphabetically")
        self.podcast_sort_menu.AppendSeparator()
        self.menu_sort_duration_long = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "&Longest First", "Sort episodes by duration, longest first")
        self.menu_sort_duration_short = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "&Shortest First", "Sort episodes by duration, shortest first")
        self.podcast_sort_menu.AppendSeparator()
        self.menu_sort_unplayed = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "&Unplayed First", "Show unplayed episodes first")
        self.menu_sort_downloaded = self.podcast_sort_menu.AppendRadioItem(wx.ID_ANY, "&Downloaded First", "Show downloaded episodes first")
        self.menu_sort_date_newest.Check(True)  # Default
        view_menu.AppendSubMenu(self.podcast_sort_menu, "&Sort Podcast Episodes")
        view_menu.AppendSeparator()
        
        # Customize home page
        self.menu_customize_home = view_menu.Append(wx.ID_ANY, "Customize &Home Page...", "Choose which widgets appear on the home page")
        view_menu.AppendSeparator()
        
        # View settings dialog
        self.menu_view_settings = view_menu.Append(wx.ID_ANY, "View &Settings...", "Configure view and appearance settings")
        ...
        # Navigation
        self.menu_skip_back = play_menu.Append(wx.ID_ANY, "Skip &Back 15s\tCtrl+Left", "Skip backward 15 seconds")
        self.menu_skip_fwd = play_menu.Append(wx.ID_ANY, "Skip &Forward 15s\tCtrl+Right", "Skip forward 15 seconds")
        play_menu.AppendSeparator()
        
        # Volume submenu
        volume_submenu = wx.Menu()
        self.menu_vol_up = volume_submenu.Append(wx.ID_ANY, "Volume &Up\tCtrl+Up", "Increase volume")
        self.menu_vol_down = volume_submenu.Append(wx.ID_ANY, "Volume &Down\tCtrl+Down", "Decrease volume")
        ...
        # Playback controls
        self.menu_playback_speed = play_menu.Append(wx.ID_ANY, "Playback S&peed...\tCtrl+P", "Adjust playback speed")
        self.menu_sleep_timer = play_menu.Append(wx.ID_ANY, "Sleep &Timer...\tCtrl+T", "Set sleep timer")
        play_menu.AppendSeparator()
        
        # Recording
        self.menu_record = play_menu.Append(wx.ID_ANY, "&Record Stream\tCtrl+R", "Start/stop recording")
        play_menu.AppendSeparator()
        
        # History
        self.menu_recently_played = play_menu.Append(wx.ID_ANY, "Recently P&layed...\tCtrl+Shift+R", "Show recently played items")
        ...
        self.menu_quick_dial_1 = quick_dial_submenu.Append(wx.ID_ANY, "Quick Dial &1\tAlt+F1", "Play favorite #1")
        self.menu_quick_dial_2 = quick_dial_submenu.Append(wx.ID_ANY, "Quick Dial &2\tAlt+F2", "Play favorite #2")
        self.menu_quick_dial_3 = quick_dial_submenu.Append(wx.ID_ANY, "Quick Dial &3\tAlt+F3", "Play favorite #3")
        self.menu_quick_dial_4 = quick_dial_submenu.Append(wx.ID_ANY, "Quick Dial &4\tAlt+F4", "Play favorite #4")
        self.menu_quick_dial_5 = quick_dial_submenu.Append(wx.ID_ANY, "Quick Dial &5\tAlt+F5", "Play favorite #5")
        ...
        for category, pods in PODCASTS.items():
            ...
            for podcast_name in pods.keys():
                ...
                self.Bind(wx.EVT_MENU, lambda e, c=category, p=podcast_name: self._play_podcast(c, p), item)
        ...
        self.menu_sync_data = tools_menu.Append(wx.ID_ANY, "Sync &Data...\tCtrl+Shift+S", "Synchronize data with ACB servers")
        tools_menu.AppendSeparator()
        
        self.menu_start_server = tools_menu.Append(wx.ID_ANY, "Start &Web Server", "Start local web server")
        self.menu_stop_server = tools_menu.Append(wx.ID_ANY, "S&top Web Server", "Stop local web server")
        tools_menu.AppendSeparator()
        
        self.menu_scheduled_tasks = tools_menu.Append(wx.ID_ANY, "Scheduled &Tasks...", "Manage scheduled tasks")
        tools_menu.AppendSeparator()
        
        self.menu_listening_stats = tools_menu.Append(wx.ID_ANY, "&Listening Statistics...", "View your listening statistics")
        self.menu_export_import = tools_menu.Append(wx.ID_ANY, "&Export/Import Settings...", "Backup or restore your settings")
        tools_menu.AppendSeparator()
        
        self.menu_auto_play = tools_menu.Append(wx.ID_ANY, "&Auto-Play on Startup...", "Configure auto-play settings")
        self.menu_quiet_hours = tools_menu.Append(wx.ID_ANY, "&Quiet Hours...", "Configure quiet hours")
        self.menu_toggle_audio_ducking = tools_menu.AppendCheckItem(wx.ID_ANY, "Enable Audio &Ducking", "Lower volume during system sounds")
        self.menu_audio_ducking_settings = tools_menu.Append(wx.ID_ANY, "Audio Ducking &Settings...", "Configure audio ducking percentage and delay")
        self.menu_toggle_media_keys = tools_menu.AppendCheckItem(wx.ID_ANY, "&Media Key Support", "Use keyboard media keys")
        tools_menu.AppendSeparator()
        
        self.menu_clear_cache = tools_menu.Append(wx.ID_ANY, "Clear &Cache...", "Clear application cache")
        self.menu_clear_recent = tools_menu.Append(wx.ID_ANY, "Clear &Recent Items...", "Clear recent items history")
        tools_menu.AppendSeparator()
        ...
        self.menu_shortcuts = help_menu.Append(wx.ID_ANY, "&Keyboard Shortcuts", "Show keyboard shortcuts")
        help_menu.AppendSeparator()
        
        # Announcements submenu
        self.menu_announcements = help_menu.Append(
            wx.ID_ANY, "&Announcements...\tCtrl+Shift+A",
            "View announcements and updates"
        )
        self.menu_whats_new = help_menu.Append(
            wx.ID_ANY, "&What's New in This Version",
            "See what's new in this version of ACB Link"
        )
        help_menu.AppendSeparator()
        
        self.menu_acb_website = help_menu.Append(wx.ID_ANY, "ACB &Website", "Visit ACB website")
        self.menu_check_updates = help_menu.Append(wx.ID_ANY, "Check for &Updates", "Check for application updates")
        help_menu.AppendSeparator()
        
        self.menu_send_feedback = help_menu.Append(wx.ID_ANY, "Send &Feedback...\tCtrl+Shift+F", "Send feedback or report an issue")
    ...
    def _build_ui(self):
        ...
        self.podcasts_panel = PodcastsPanel(
            self.notebook,
            self._play_podcast_episode
        )
        self.affiliates_panel = AffiliatesPanel(
            self.notebook,
            on_suggest_correction=self._on_suggest_affiliate_correction
        )
    ...
    def _build_player_controls(self, parent) -> wx.Panel:
        ...
        controls_sizer.Add(now_playing_sizer, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, UIConstants.PADDING_MEDIUM)
        ...
        make_button_accessible(self.btn_skip_back, "Skip Backward", "Ctrl+Left", 
        ...
        make_button_accessible(self.btn_skip_fwd, "Skip Forward", "Ctrl+Right",
        ...
        controls_sizer.Add(playback_sizer, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, UIConstants.PADDING_SMALL)
        controls_sizer.AddStretchSpacer()
        ...
        controls_sizer.Add(volume_sizer, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, UIConstants.PADDING_MEDIUM)
        
        # Record button (far right) with accessible name
        self.btn_record = wx.Button(panel, label="⏺", size=btn_size)
        make_button_accessible(self.btn_record, "Record", "Ctrl+R", "Start or stop recording current stream")
        self.btn_record.Bind(wx.EVT_BUTTON, self._on_record_toggle)
        self.btn_record.SetForegroundColour(hex_to_colour(scheme.error))
        controls_sizer.Add(self.btn_record, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, UIConstants.PADDING_MEDIUM)
    ...
    def _setup_accelerators(self):
        ...
        accel_entries = [
            ...
            (wx.ACCEL_CTRL | wx.ACCEL_SHIFT, ord('E'), self.menu_suggest_affiliate_correction.GetId()),
    ...
    def _bind_events(self):
        ...
        self.Bind(wx.EVT_MENU, lambda e: self._on_podcast_sort("date_newest"), self.menu_sort_date_newest)
        self.Bind(wx.EVT_MENU, lambda e: self._on_podcast_sort("date_oldest"), self.menu_sort_date_oldest)
        ...
        self.Bind(wx.EVT_MENU, lambda e: self._on_podcast_sort("duration_longest"), self.menu_sort_duration_long)
        self.Bind(wx.EVT_MENU, lambda e: self._on_podcast_sort("duration_shortest"), self.menu_sort_duration_short)
        self.Bind(wx.EVT_MENU, lambda e: self._on_podcast_sort("unplayed_first"), self.menu_sort_unplayed)
        self.Bind(wx.EVT_MENU, lambda e: self._on_podcast_sort("downloaded_first"), self.menu_sort_downloaded)
        ...
        self.Bind(wx.EVT_MENU, self._on_suggest_affiliate_correction_menu, self.menu_suggest_affiliate_correction)
    ...
    def _init_announcement_system(self):
        ...
        try:
            from .announcements import initialize_announcement_manager, get_announcement_manager
            ...
            self.announcement_manager.add_new_announcements_callback(
                self._on_new_announcements
            )
    ...
    def _check_announcements_on_startup(self):
        ...
        if settings.check_on_startup:
            self.announcement_manager.fetch_announcements(
                callback=self._on_announcements_fetched
            )
        ...
        try:
            ...
            if status:
                ...
                if status.total_changes > 0:
                    ...
                    message += f"Sources updated: {status.successful_sources}/{status.total_sources}\n"
                    ...
                    wx.MessageBox("All data is up to date.", "Sync Complete", wx.OK | wx.ICON_INFORMATION)
    ...
    def _on_scheduled_tasks(self, event):
        ...
        try:
            ...
            wx.MessageBox("No scheduled tasks found.", "Scheduled Tasks", wx.OK | wx.ICON_INFORMATION)
    
    def _on_clear_cache(self, event):
        """Clear application cache."""
        if wx.MessageBox("Clear all cached data?", "Clear Cache", wx.YES_NO | wx.ICON_QUESTION) == wx.YES:
    ...
    def _on_clear_recent(self, event):
        """Clear recent items."""
        if wx.MessageBox("Clear recent items history?", "Clear History", wx.YES_NO | wx.ICON_QUESTION) == wx.YES:
    ...
    def _on_affiliate_admin(self, event):
        ...
        try:
            ...
            wx.MessageBox(
                "Affiliate admin module is not available.",
                "Error",
                wx.OK | wx.ICON_ERROR
            )
        except Exception as e:
            ...
            wx.MessageBox(
                f"Error opening affiliate admin: {e}",
                "Error",
                wx.OK | wx.ICON_ERROR
            )
    ...
    def _on_recently_played(self, event):
        ...
        dialog = RecentlyPlayedDialog(
            self,
            self.recently_played,
            self._play_from_recent
        )
        dialog.ShowModal()
        dialog.Destroy()
...
"""
    ...
    def _on_whats_new(self, event):
        ...
        try:
            ...
            for announcement in manager.get_all_announcements(include_expired=True, include_read=True):
    ...
    def _on_customize_home(self, event):
        ...
        from .home_widgets import HomePageCustomizeDialog, HomePageSettings
