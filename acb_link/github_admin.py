...
class GitHubClient:
    ...
    def _request(
        ...
        headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "ACBLinkDesktop/1.1"}
    ...
    def get_repo_permission(self, owner: str, repo: str, username: str) -> Tuple[bool, Any]:
    ...
    def update_file(
        ...
        return self._request(f"/repos/{owner}/{repo}/contents/{path}", method="PUT", data=data)
...
class GitHubAdminManager:
    ...
    def authenticate(self, github_token: str) -> Tuple[bool, str, Optional[AdminSession]]:
        ...
        user = GitHubUser(
            username=username, display_name=display_name, email=email, avatar_url=avatar
        )
    ...
    def logout(self):
        ...
        if self._session:
            self._log_action(
                "logout", self._session.username, self._session.role.name, "session", {}
            )
        self._session = None
    ...
    def require_role(self, required_role: AdminRole) -> Tuple[bool, str]:
        ...
        return (
            False,
            f"Insufficient permissions. Required: {required_role.name}, Have: {self._session.role.name}",
        )
    ...
    def fetch_org_config(self) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        success, data = client.get_file_contents(
            self.repo_owner, self.repo_name, self.config_file, self.config_branch
        )
    ...
    def update_org_config(self, config: Dict[str, Any], commit_message: str) -> Tuple[bool, str]:
        """
    ...
    def _save_cached_config(self, config: Dict[str, Any], sha: str):
        ...
        cache_path = self.data_dir / CONFIG_CACHE_FILE
        try:
            cache_data = {"config": config, "_cached_at": datetime.now().isoformat(), "_sha": sha}
