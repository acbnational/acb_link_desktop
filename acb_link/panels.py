    ...
    def _add_podcasts_section(self):
        ...
        for category, pods in PODCASTS.items():
            ...
            for podcast_name in pods.keys():
                ...
                btn.Bind(
                    wx.EVT_BUTTON, lambda e, c=category, p=podcast_name: self.on_play_podcast(c, p)
                )
...
class StreamsPanel(wx.Panel):
    ...
    def _build_ui(self):
        ...
        self.list_ctrl = wx.ListCtrl(self, style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.BORDER_SIMPLE)
        self.list_ctrl.SetFont(
            wx.Font(11, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL)
        )
        make_list_accessible(
            self.list_ctrl, "ACB Media Streams list", "Select a stream and press Enter to play"
        )
        ...
        make_accessible(
            self.btn_record, "Record Stream", "Start or stop recording the selected stream"
        )
...
class PodcastsPanel(wx.Panel):
    ...
    def _build_ui(self):
        ...
        self.tree = wx.TreeCtrl(
            left_panel, style=wx.TR_HAS_BUTTONS | wx.TR_SINGLE | wx.BORDER_SUNKEN
        )
        ...
        make_list_accessible(
            self.episode_list, "Podcast episodes list", "Select an episode and press Enter to play"
        )
        ...
        make_accessible(
            self.btn_download,
            "Download Episode",
            "Download the selected episode for offline listening",
        )
        ...
        make_accessible(
            self.btn_refresh, "Refresh Feed", "Refresh the podcast feed to check for new episodes"
        )
    ...
    def _on_refresh(self, event):
        ...
        if podcast:
            ...
            wx.MessageBox(
                f"Refreshing feed for {podcast['name']}...", "Refresh", wx.OK | wx.ICON_INFORMATION
            )
...
class ResourcesPanel(wx.Panel):
    ...
    def _build_ui(self):
        ...
        make_list_accessible(
            self.list_ctrl,
            "ACB Resources list",
            "Select a resource and press Enter to open in browser",
        )
...
class AffiliatesPanel(wx.Panel):
    ...
    def _build_ui(self):
        ...
        make_accessible(
            self.btn_copy, "Copy Info", "Copy all information about the selected affiliate"
        )
        ...
        make_accessible(
            self.btn_email, "Send Email", "Open your email client to contact the affiliate"
        )
    ...
    def _build_states_tab(self):
        ...
        self.states_list = wx.ListCtrl(
            self.states_panel, style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.BORDER_SUNKEN
        )
        make_list_accessible(
            self.states_list,
    ...
    def _build_sigs_tab(self):
        ...
        self.sigs_list = wx.ListCtrl(
            self.sigs_panel, style=wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.BORDER_SUNKEN
        )
        make_list_accessible(
            self.sigs_list,
    ...
    def _show_correction_dialog(self):
        ...
        try:
            ...
            wx.MessageBox(
                f"Could not open correction dialog: {e}", "Error", wx.OK | wx.ICON_ERROR, self
            )
    ...
    def _on_context_menu(self, event):
        ...
        if affiliate.get("twitter") or affiliate.get("facebook"):
            ...
            if affiliate.get("twitter"):
                ...
                menu.Bind(
                    wx.EVT_MENU, lambda e: self._open_social("twitter", affiliate), twitter_item
                )

            if affiliate.get("facebook"):
                fb_item = social_menu.Append(wx.ID_ANY, "Open &Facebook")
                menu.Bind(wx.EVT_MENU, lambda e: self._open_social("facebook", affiliate), fb_item)
