...
class VoiceController:
    ...
    def remove_triggers(self, command_name: str, triggers: List[str]) -> bool:
        ...
        if command_name in self.commands:
            ...
            self.commands[command_name].triggers = [t for t in current if t not in triggers]
            return True
        return False
    ...
    def get_loaded_plugins(self) -> List[Dict[str, str]]:
        ...
        return [
            {"name": p.name, "version": p.version, "author": p.author, "description": p.description}
    ...
    def save_config(self, path: Optional[str] = None) -> bool:
        ...
        config = {
            "commands": {
                name: {"enabled": cmd.enabled, "triggers": cmd.triggers, "priority": cmd.priority}
    ...
    def search_commands(self, query: str) -> List[VoiceCommand]:
        ...
        for cmd in self.commands.values():
            if (
                ...
                or any(query in t.lower() for t in cmd.triggers)
            ):
                results.append(cmd)
        return results
    ...
    def _reset_wake_word_timer(self):
        ...
        self._wake_word_timer = threading.Timer(self.wake_word_timeout, self._on_wake_word_timeout)
    ...
    def _handle_recognized_text(self, text: str):
        ...
        sorted_commands = sorted(
            [c for c in self.commands.values() if c.enabled], key=lambda c: c.priority, reverse=True
        )
    ...
    def get_command_list(self) -> List[Dict[str, str]]:
        ...
        return [
            {"name": cmd.name, "triggers": cmd.triggers, "description": cmd.description}
            for cmd in self.commands.values()
        ]
...
class VoiceControlManager:
        ...
        for i in range(1, 11):
            if i <= len(stream_names):
                ...
                vc.set_command_action(
                    f"play_stream_{i}", lambda name=stream_name: mf._play_stream(name)
                )
    ...
    def _on_voice_command(self, command_name: str, full_text: str):
        ...
        if hasattr(self.main_frame, "statusbar"):
            ...
            wx.CallAfter(self.main_frame.statusbar.SetStatusText, f"Voice: {command_name}", 0)
