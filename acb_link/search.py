...
class GlobalSearch:
    ...
    def _perform_search(self, query: SearchQuery) -> List[SearchResult]:
        ...
        if not query.types or SearchResultType.PODCAST in query.types or SearchResultType.EPISODE in query.types:
                    ...
                    for episode in getattr(podcast, 'episodes', []):
                        ...
                        if regex.search(ep_title) or regex.search(ep_desc):
                            ...
                            results.append(SearchResult(
                                ...
                                title=ep_title,
                                description=ep_desc[:200] + "..." if len(ep_desc) > 200 else ep_desc,
                                url=getattr(episode, 'url', ''),
                                relevance_score=self._calculate_relevance(search_text, ep_title, ep_desc),
