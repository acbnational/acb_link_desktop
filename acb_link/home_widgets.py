...
class HomePageSettings:
    ...
    def save(self):
        ...
        try:
            data = {
                'widgets': [
                    {**asdict(w), 'widget_type': w.widget_type.value}
    ...
    def get_enabled_widgets(self) -> List[WidgetConfig]:
        ...
        return sorted(
            [w for w in self.widgets if w.enabled],
    ...
    def move_widget(self, widget_type: WidgetType, direction: int):
        ...
        for i, w in enumerate(enabled):
            if w.widget_type == widget_type:
                ...
                if new_idx != i:
                    # Swap orders
                    enabled[i].order, enabled[new_idx].order = enabled[new_idx].order, enabled[i].order
                    self.save()
                break
...
class BaseWidget(wx.Panel):
    ...
    def _build_ui(self):
        ...
        self.heading = wx.StaticText(
            self.header_panel,
            label=self.config.display_title
        )
        ...
        self.collapse_btn = wx.Button(
            self.header_panel,
            label="−" if not self._collapsed else "+",
            size=(28, 28)
        )
        self.collapse_btn.SetToolTip("Collapse section" if not self._collapsed else "Expand section")
...
class FavoriteStreamsWidget(BaseWidget):
    ...
    def _build_content(self):
        ...
        self.empty_label = wx.StaticText(
            self.content_panel,
            label="No favorite streams yet. Add streams from the Streams tab."
        )
...
class FavoriteAffiliatesWidget(BaseWidget):
    ...
    def _build_content(self):
        ...
        self.empty_label = wx.StaticText(
            self.content_panel,
            label="Enable this in Affiliates tab to see your state or SIG here."
        )
        self.empty_label.SetForegroundColour(wx.Colour(102, 102, 102))
        self.content_sizer.Add(self.empty_label, 0, wx.ALL, 5)
...
class NowPlayingWidget(BaseWidget):
    ...
    def _build_content(self):
        ...
        self.now_playing_label.SetFont(
            self.now_playing_label.GetFont().Bold()
        )
        self.content_sizer.Add(self.now_playing_label, 0, wx.ALL, 5)
        
        self.details_label = wx.StaticText(
            self.content_panel,
            label="Select a stream or podcast to start listening"
        )
...
class HomePageCustomizeDialog(wx.Dialog):
    ...
    def _build_ui(self):
        ...
        self.widgets_list = wx.CheckListBox(
            panel,
            size=(-1, 250),
            style=wx.LB_SINGLE
        )
    ...
    def _on_move_up(self, event):
        ...
        sorted_widgets[idx].order, sorted_widgets[idx - 1].order = \
            sorted_widgets[idx - 1].order, sorted_widgets[idx].order
