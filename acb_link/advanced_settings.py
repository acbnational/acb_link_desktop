...
class FieldValidator:
    ...
    def validate_integer(
        value: Any, field_name: str, min_val: Optional[int] = None, max_val: Optional[int] = None
    ) -> Tuple[bool, str]:
    ...
    @staticmethod
    def validate_choice(value: str, field_name: str, choices: List[str]) -> Tuple[bool, str]:
...
class AdvancedSettingsWarningDialog(wx.Dialog):
    ...
    def _build_ui(self):
        ...
        make_button_accessible(
            btn_proceed, "Proceed to Advanced Settings", "Open the advanced settings dialog"
        )
        ...
        announce(
            "Warning: You are about to access Advanced Settings. "
...
class AdvancedSettingsDialog(wx.Dialog):
    ...
    def _build_ui(self):
        ...
        warning_icon = wx.StaticBitmap(
            header, bitmap=wx.ArtProvider.GetBitmap(wx.ART_TIP, wx.ART_TOOLBAR, (24, 24))
        )
        header_sizer.Add(warning_icon, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)

        header_text = wx.StaticText(
            header, label="Expert Mode: Changes here affect core application behavior"
        )
    ...
    def _add_data_sources_tab(self):
        ...
        base_row.Add(
            wx.StaticText(panel, label="Base URL:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        ...
        sources = [
            ...
            (
                ...
                self.config.live_data.special_interest_groups,
            ),
        ...
        for key, label, source in sources:
            ...
            source_sizer_inner.Add(
                wx.StaticText(source_panel, label=f"{label} URL:"), 0, wx.ALIGN_CENTER_VERTICAL
            )
            ...
            make_accessible(
                enabled_chk, f"{label} enabled", f"Enable or disable {label.lower()} data fetching"
            )
            source_sizer_inner.Add(enabled_chk, 0, wx.ALIGN_CENTER_VERTICAL)

            source_sizer_inner.Add(
                wx.StaticText(source_panel, label="Cache (hours):"), 0, wx.ALIGN_CENTER_VERTICAL
            )
            cache_ctrl = wx.SpinCtrl(
                source_panel, value=str(source.cache_duration_hours), min=1, max=720
            )
            make_accessible(
                cache_ctrl, f"{label} cache duration", "Hours to cache data before refreshing"
            )
    ...
    def _add_network_tab(self):
        ...
        timeout_row.Add(
            wx.StaticText(panel, label="Connection timeout:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.timeout = wx.SpinCtrl(
            panel, value=str(self.config.network.timeout_seconds), min=5, max=120
        )
        ...
        retry_row.Add(
            wx.StaticText(panel, label="Retry count:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.retry_count = wx.SpinCtrl(
            panel, value=str(self.config.network.retry_count), min=0, max=10
        )
        make_accessible(
            self.retry_count, "Retry count", "Number of times to retry failed connections"
        )
        retry_row.Add(self.retry_count, 0)
        retry_row.Add(
            wx.StaticText(panel, label="(0 = no retries)"),
            0,
        ...
        delay_row.Add(
            wx.StaticText(panel, label="Retry delay:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.retry_delay = wx.SpinCtrl(
            panel, value=str(self.config.network.retry_delay_seconds), min=1, max=60
        )
        ...
        make_accessible(
            self.user_agent, "User agent string", "Identifier sent to servers when connecting"
        )
        ...
        make_accessible(
            self.proxy_enabled, "Use proxy server", "Route connections through a proxy server"
        )
        proxy_sizer.Add(self.proxy_enabled, 0, wx.ALL, 5)

        proxy_row = wx.BoxSizer(wx.HORIZONTAL)
        proxy_row.Add(
            wx.StaticText(panel, label="Proxy URL:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.proxy_url = wx.TextCtrl(
            panel, value=self.config.network.proxy_url or "", size=(400, -1)
        )
        make_accessible(
            self.proxy_url, "Proxy URL", "Proxy server address, e.g., http://proxy:8080"
        )
        proxy_row.Add(self.proxy_url, 1)
        proxy_sizer.Add(proxy_row, 0, wx.EXPAND | wx.ALL, 5)

        # Enable/disable proxy URL based on checkbox
        self.proxy_enabled.Bind(
            wx.EVT_CHECKBOX, lambda e: self.proxy_url.Enable(self.proxy_enabled.GetValue())
        )
    ...
    def _add_paths_tab(self):
        ...
        btn_open_data.Bind(
            wx.EVT_BUTTON, lambda e: self._open_folder(self.path_controls["app_data"].GetValue())
        )
        make_button_accessible(
            btn_open_data, "Open App Data Folder", "Open the app data folder in Explorer"
        )
        open_sizer.Add(btn_open_data, 0, wx.ALL, 5)

        btn_open_downloads = wx.Button(panel, label="Open Downloads Folder")
        btn_open_downloads.Bind(
            wx.EVT_BUTTON, lambda e: self._open_folder(self.path_controls["downloads"].GetValue())
        )
        make_button_accessible(
            btn_open_downloads, "Open Downloads Folder", "Open the downloads folder in Explorer"
        )
    ...
    def _add_updates_tab(self):
        ...
        interval_row.Add(
            wx.StaticText(panel, label="Check interval:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.check_interval = wx.SpinCtrl(
            panel, value=str(self.config.updates.check_interval_hours), min=1, max=168
        )
        make_accessible(
            self.check_interval, "Update check interval", "Hours between automatic update checks"
        )
        interval_row.Add(self.check_interval, 0)
        interval_row.Add(
            wx.StaticText(panel, label="hours"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 5
        )
        ...
        repo_row.Add(
            wx.StaticText(panel, label="GitHub repository:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.github_repo = wx.TextCtrl(panel, value=self.config.updates.github_repo, size=(300, -1))
        make_accessible(
            self.github_repo, "GitHub repository", "Repository name in owner/repo format"
        )
        ...
        api_row.Add(
            wx.StaticText(panel, label="API URL:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.github_api = wx.TextCtrl(
            panel, value=self.config.updates.github_api_url, size=(400, -1)
        )
        make_accessible(self.github_api, "GitHub API URL", "URL for checking releases")
        ...
        self.skipped_label = wx.StaticText(panel, label=f"Currently skipped version: {skipped}")
        ...
        make_button_accessible(
            btn_clear_skip, "Clear Skipped Version", "Allow the skipped version to be offered again"
        )
    ...
    def _add_defaults_tab(self):
        ...
        self.default_volume = wx.SpinCtrl(
            panel, value=str(self.config.defaults.default_volume), min=0, max=100
        )
        make_accessible(self.default_volume, "Default volume", "Volume level 0-100")
        vol_row.Add(self.default_volume, 0)
        vol_row.Add(wx.StaticText(panel, label="%"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 5)
        audio_sizer.Add(vol_row, 0, wx.ALL, 5)

        speed_row = wx.BoxSizer(wx.HORIZONTAL)
        speed_row.Add(
            wx.StaticText(panel, label="Default playback speed:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.default_speed = wx.Choice(
            panel, choices=["0.5x", "0.75x", "1.0x", "1.25x", "1.5x", "1.75x", "2.0x"]
        )
        speed_map = {0.5: 0, 0.75: 1, 1.0: 2, 1.25: 3, 1.5: 4, 1.75: 5, 2.0: 6}
        self.default_speed.SetSelection(
            speed_map.get(self.config.defaults.default_playback_speed, 2)
        )
        make_accessible(self.default_speed, "Default playback speed", "Speed for audio playback")
        ...
        self.skip_forward = wx.SpinCtrl(
            panel, value=str(self.config.defaults.default_skip_forward), min=5, max=120
        )
        ...
        self.skip_backward = wx.SpinCtrl(
            panel, value=str(self.config.defaults.default_skip_backward), min=5, max=120
        )
        ...
        tab_row.Add(
            wx.StaticText(panel, label="Default tab:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.default_tab = wx.Choice(
            panel,
            choices=[
                "Home",
                "Streams",
        ...
        theme_row.Add(
            wx.StaticText(panel, label="Default theme:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 10
        )
        self.default_theme = wx.Choice(
            panel, choices=["System", "Light", "Dark", "High Contrast Light", "High Contrast Dark"]
        )
        theme_map = {
            "system": 0,
            "light": 1,
        ...
        font_row.Add(
            wx.StaticText(panel, label="Default font size:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            10,
        )
        self.default_font_size = wx.SpinCtrl(
            panel, value=str(self.config.defaults.default_font_size), min=8, max=32
        )
        make_accessible(self.default_font_size, "Default font size", "Font size in points")
        font_row.Add(self.default_font_size, 0)
        font_row.Add(wx.StaticText(panel, label="pt"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 5)
        ...
        make_accessible(
            self.start_minimized, "Start minimized", "Start app minimized to system tray"
        )
        ...
        make_accessible(
            self.minimize_to_tray, "Minimize to system tray", "Minimize to tray instead of taskbar"
        )
    ...
    def _add_experimental_tab(self):
        ...
        make_accessible(
            self.debug_logging, "Enable debug logging", "Write detailed logs for troubleshooting"
        )
        ...
        make_accessible(self.show_perf, "Show performance metrics", "Display memory and CPU usage")
        ...
        make_button_accessible(btn_export, "Export Configuration", "Save all settings to a file")
        ...
        make_button_accessible(
            btn_import, "Import Configuration", "Load settings from a backup file"
        )
    ...
    def _create_buttons(self) -> wx.BoxSizer:
        ...
        make_button_accessible(
            btn_reset, "Reset All to Defaults", "Restore all advanced settings to original values"
        )
        ...
        make_button_accessible(
            btn_validate, "Validate", "Check all settings for errors without saving"
        )
        ...
        make_button_accessible(btn_ok, "Save Changes", "Save all advanced settings")

        btn_cancel = wx.Button(self, wx.ID_CANCEL, "Cancel")
        make_button_accessible(btn_cancel, "Cancel", "Close without saving changes")
    ...
    def _browse_folder(self, text_ctrl: wx.TextCtrl):
        ...
        dlg = wx.DirDialog(
            self, "Choose a directory:", defaultPath=text_ctrl.GetValue(), style=wx.DD_DEFAULT_STYLE
        )
            ...
            if dlg.ShowModal() == wx.ID_OK:
                if (
                    ...
                    try:
                        ...
                        wx.MessageBox(
                            "Configuration imported. Please restart "
    ...
    def _on_validate(self, event):
        ...
        if errors:
            ...
            wx.MessageBox(
                "All settings are valid.", "Validation Passed", wx.OK | wx.ICON_INFORMATION
            )
    ...
    def _validate_all(self) -> List[str]:
        ...
        if self.proxy_enabled.GetValue():
            valid, msg = FieldValidator.validate_url(self.proxy_url.GetValue(), "Proxy URL")
            if not valid:
                errors.append(msg)
    ...
    def _on_reset_defaults(self, event):
        ...
        if (
            ...
            self.default_theme.SetSelection(theme_map.get(default_config.defaults.default_theme, 0))
    ...
    def _on_save(self, event):
        ...
        if save_config():
            ...
            wx.MessageBox("Failed to save configuration file.", "Save Error", wx.OK | wx.ICON_ERROR)
    ...
    def _apply_to_config(self):
        ...
        self.config.defaults.default_playback_speed = speed_values[
            self.default_speed.GetSelection()
        ]
...
def show_advanced_settings(
    parent, config: AppConfig, settings: AppSettings, skip_warning: bool = False
) -> bool:
    """
    ...
    try:
        ...
        success, session = require_admin_login(
            parent, AdminRole.CONFIG_ADMIN, "modifying advanced settings"
        )
        if not success:
    ...
    if not skip_warning:
        ...
        _should_show_again = warning.should_show_again()  # noqa: F841 - TODO: persist preference
        warning.Destroy()
        ...
        # settings.show_advanced_warning = _should_show_again
