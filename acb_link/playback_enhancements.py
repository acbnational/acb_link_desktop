...
class EnhancedSleepTimer:
    ...
    def _timer_loop(self):
        ...
        while self._remaining_seconds > 0 and not self._stop_event.is_set():
            ...
            if (self._enable_fade and 
                self._remaining_seconds <= self._fade_duration_seconds and 
                not fade_started):
                fade_started = True
...
class SleepTimerDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        preset_values = [
            ...
            ("1 hour", 60), ("90 min", 90), ("2 hours", 120)
        ]
...
class AutoPlayManager:
    ...
    def __init__(self, settings_path: Optional[str] = None):
        self.settings_path = settings_path or str(
            Path.home() / ".acb_link" / "autoplay.json"
        )
...
class RecentlyPlayedManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "recently_played.json"
        )
    ...
    def add(
        ...
        self.items.insert(0, RecentItem(
...
class AudioDuckingManager:
    ...
    def __init__(
        ...
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "audio_ducking.json"
        )
    ...
    def duck(self, duration_seconds: Optional[float] = None):
        ...
        duration = duration_seconds if duration_seconds is not None else self.settings.restore_delay_seconds
...
class AudioDuckingDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        self.percent_spin = wx.SpinCtrl(
            panel, min=10, max=90, initial=self.manager.duck_percentage
        )
        self.percent_spin.SetName("Duck volume percentage")
        percent_sizer.Add(self.percent_spin, 0, wx.RIGHT, 5)
        percent_sizer.Add(
            wx.StaticText(panel, label="% of current volume"),
        ...
        delay_sizer.Add(
            wx.StaticText(panel, label="seconds"),
            0, wx.ALIGN_CENTER_VERTICAL
        )
...
class QuietHoursManager:
    ...
    def __init__(
        ...
        self.settings_path = settings_path or str(
            Path.home() / ".acb_link" / "quiet_hours.json"
        )
...
class QuietHoursDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        # Options
        self.chk_mute_notifications = wx.CheckBox(panel, label="&Mute notifications during quiet hours")
        ...
        vol_sizer.Add(wx.StaticText(panel, label="Reduced volume:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.vol_slider = wx.Slider(panel, value=self.manager.settings.reduced_volume_percent, minValue=10, maxValue=100)
        self.vol_slider.SetName("Reduced volume percent")
        vol_sizer.Add(self.vol_slider, 1)
        self.vol_label = wx.StaticText(panel, label=f"{self.manager.settings.reduced_volume_percent}%")
...
class PlaybackSpeedDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        self.speed_label = wx.StaticText(
            panel,
            label=f"Current speed: {self.speed_controller.get_label()}"
        )
...
class AutoPlayDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        type_sizer.Add(wx.StaticText(panel, label="Play:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        
        self.content_type = wx.Choice(panel, choices=["Last played", "Specific stream"])
        self.content_type.SetSelection(0 if self.manager.settings.content_type == "last_played" else 1)
        ...
        stream_sizer.Add(wx.StaticText(panel, label="Stream:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        ...
        # Delay setting
        delay_sizer = wx.BoxSizer(wx.HORIZONTAL)
        delay_sizer.Add(wx.StaticText(panel, label="Delay before playing:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        self.delay_spin = wx.SpinCtrl(panel, min=0, max=30, initial=self.manager.settings.delay_seconds)
        delay_sizer.Add(self.delay_spin, 0)
        delay_sizer.Add(wx.StaticText(panel, label="seconds"), 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 5)
        ...
        self.chk_only_if_playing = wx.CheckBox(
            panel,
            label="Only auto-play if &previous session was playing"
        )
    ...
    def _on_ok(self, event):
        ...
        self.manager.settings.only_when_last_session_was_playing = self.chk_only_if_playing.GetValue()
        self.manager.save()
        self.EndModal(wx.ID_OK)
