...
from .home_widgets import (
    HomePageSettings, WidgetType, WidgetConfig, WIDGET_CLASSES,
    WIDGET_TITLES, WIDGET_DESCRIPTIONS, HomePageCustomizeDialog,
    WelcomeWidget, FavoriteStreamsWidget, FavoritePodcastsWidget,
    CalendarEventsWidget, FavoriteAffiliatesWidget, QuickActionsWidget,
    NowPlayingWidget, RecentEpisodesWidget, DownloadProgressWidget,
    ListeningStatsWidget, BaseWidget
)
...
class CustomizableHomePanel(wx.Panel):
    ...
    def _create_header(self) -> wx.Panel:
        ...
        self.subtitle = wx.StaticText(
            panel,
            label="Your personalized gateway to ACB media content"
        )
    ...
    def _create_widget(self, config: WidgetConfig) -> Optional[BaseWidget]:
        ...
        if widget_type == WidgetType.FAVORITE_STREAMS:
            ...
            return DownloadProgressWidget(
                self.scroll, config, self._handle_action,
                self.podcast_manager.download_manager
            )
