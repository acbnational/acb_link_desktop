...
class BookmarkManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "bookmarks.json"
        )
...
class QuickSeekDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        info = wx.StaticText(
            panel,
            label=f"Current: {cur_mins}:{cur_secs:02d} / Duration: {dur_mins}:{dur_secs:02d}"
        )
        ...
        time_sizer.Add(wx.StaticText(panel, label="Go to:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        ...
        for label, seconds in [("-30s", -30), ("-10s", -10), ("+10s", 10), ("+30s", 30), ("+1m", 60)]:
...
class AudioProfileManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "audio_profiles.json"
        )
...
class AudioProfileDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        profile_sizer.Add(wx.StaticText(panel, label="Profile:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        ...
        self.bass_slider.Bind(wx.EVT_SLIDER, lambda e: self.bass_label.SetLabel(f"{self.bass_slider.GetValue():+d}"))
        ...
        self.mid_slider.Bind(wx.EVT_SLIDER, lambda e: self.mid_label.SetLabel(f"{self.mid_slider.GetValue():+d}"))
        ...
        self.treble_slider.Bind(wx.EVT_SLIDER, lambda e: self.treble_label.SetLabel(f"{self.treble_slider.GetValue():+d}"))
...
class EpisodeTracker:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "episode_status.json"
        )
...
class ListeningGoalManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "listening_goals.json"
        )
...
class ListeningGoalDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        self.daily_label = wx.StaticText(panel, label=f"{daily_current}/{daily_target} min ({daily_pct}%)")
        ...
        daily_set_sizer.Add(wx.StaticText(panel, label="Target:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        ...
        self.weekly_label = wx.StaticText(panel, label=f"{weekly_current}/{weekly_target} min ({weekly_pct}%)")
        ...
        weekly_set_sizer.Add(wx.StaticText(panel, label="Target:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
...
class QuickNotesManager:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = storage_path or str(
            Path.home() / ".acb_link" / "quick_notes.json"
        )
    ...
    def get_notes_for_content(
        ...
        return [
            ...
            if (n.content_type == content_type and
                n.content_name == content_name and
                n.episode_title == episode_title)
        ]
...
class CrossfadeManager:
    ...
    def start_fade_out(self, on_complete: Callable[[], None]):
        ...
        self._fade_thread = threading.Thread(
            ...
            args=(self._original_volume, 0, on_complete, True),
            daemon=True
        )
...
class DownloadQueueManager:
    ...
    def __init__(
        ...
        self.download_folder = download_folder or str(
            Path.home() / ".acb_link" / "downloads"
        )
