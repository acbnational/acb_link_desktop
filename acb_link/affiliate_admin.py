        ...
        if self.queue_file.exists():
            try:
                with open(self.queue_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self._corrections = [
                        AffiliateCorrection.from_dict(item) for item in data.get("corrections", [])
                    ]
...
class AuditLog:
        ...
        if self.log_file.exists():
            try:
                with open(self.log_file, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    self._entries = [self.Entry.from_dict(item) for item in data.get("entries", [])]
            except (json.JSONDecodeError, KeyError):
                self._entries = []

    def _save(self):
        """Save log to file."""
        data = {"version": "1.0", "entries": [e.to_dict() for e in self._entries]}
        with open(self.log_file, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)

    def log(self, action: str, correction: AffiliateCorrection, performed_by: str, notes: str = ""):
    ...
    def get_history(self, affiliate_name: Optional[str] = None, limit: int = 100) -> List[Entry]:
...
class XMLUpdater:
    ...
    def _find_affiliate_element(
        self, root: ET.Element, affiliate_name: str
    ) -> Optional[ET.Element]:
    ...
    def get_current_values(self, affiliate_name: str, affiliate_type: str) -> Dict[str, str]:
        """
    ...
    def apply_correction(
        self, correction: AffiliateCorrection, create_backup: bool = True
    ) -> tuple[bool, str]:
        """
...
class CorrectionManager:
    ...
    def __init__(self, data_dir: Optional[Path] = None, xml_dir: Optional[Path] = None):
        """
    ...
    def submit_correction(
        ...
        original_values = self.updater.get_current_values(affiliate_name, affiliate_type)
    ...
    def approve_correction(
        self, correction_id: str, reviewer: str, notes: str = ""
    ) -> tuple[bool, str]:
        """
    ...
    def reject_correction(self, correction_id: str, reviewer: str, reason: str) -> tuple[bool, str]:
    ...
    def apply_correction(self, correction_id: str, applier: str) -> tuple[bool, str]:
        """
...
class CorrectionDetailsDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        affiliated_label = wx.StaticText(
            panel, label=f"Submitter affiliated with organization: {affiliated_text}"
        )
...
class AdminReviewPanel(wx.Panel):
    ...
    def __init__(
        self, parent: wx.Window, manager: CorrectionManager, admin_name: Optional[str] = None
    ):
        """
    ...
    def _create_ui(self):
        ...
        # Header
        header = wx.StaticText(self, label="Review Affiliate Corrections")
        ...
        self.filter_choice = wx.Choice(
            self, choices=["Pending", "Approved", "Rejected", "Applied", "All"]
        )
    ...
    def _refresh_list(self):
        ...
        self.status_text.SetLabel(f"Pending: {pending} | Approved (ready to apply): {approved}")
    ...
    def _on_approve(self, event):
        ...
        if result == wx.YES:
            success, message = self.manager.approve_correction(correction.id, self.admin_name)
            if success:
                wx.MessageBox(
    ...
    def _on_reject(self, event):
        ...
        dialog = wx.TextEntryDialog(self, "Enter reason for rejection:", "Reject Correction", "")
    ...
    def _on_apply(self, event):
        ...
        if result == wx.YES:
            success, message = self.manager.apply_correction(correction.id, self.admin_name)
            if success:
                wx.MessageBox(
...
def get_correction_manager(
    data_dir: Optional[Path] = None, xml_dir: Optional[Path] = None
) -> CorrectionManager:
    """
