...
class EventReminder:
    ...
    def set_reminder(self, event: CalendarEvent, minutes_before: int = 15):
        ...
        timer = threading.Timer(delay, self._fire_reminder, args=(event,))
        timer.daemon = True
        timer.start()
...
class CalendarManager:
    ...
    def refresh_events(
        ...
        thread = threading.Thread(
            target=self._fetch_events, args=(on_complete, on_error), daemon=True
        )
        thread.start()
    ...
    def start_monitoring(self, check_interval: int = 60):
        ...
        self._refresh_thread = threading.Thread(
            target=self._monitor_loop, args=(check_interval,), daemon=True, name="CalendarMonitor"
        )
