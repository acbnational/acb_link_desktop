...
def build_affiliate_correction_url(
    ...
    for field_key, new_value in corrections.items():
        field_label = next(
            (label for key, label in CORRECTION_FIELDS if key == field_key), field_key
        )
...
class AffiliateCorrectionDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        self.affiliate_choice = wx.Choice(panel, choices=[aff.name for aff in self.affiliates])
        ...
        corrections_label = wx.StaticText(
            scroll, label="What would you like to correct? (check all that apply)"
        )
        corrections_label.SetFont(corrections_label.GetFont().Bold())
        ...
        self.notes_text = wx.TextCtrl(
            panel, style=wx.TE_MULTILINE, size=(-1, 60)  # type: ignore[arg-type]
        )
        ...
        self.affiliated_checkbox = wx.CheckBox(
            panel, label="&I am affiliated with this organization"
        )
    ...
    def _bind_events(self):
        ...
        for field_key, checkbox in self.correction_checkboxes.items():
            checkbox.Bind(
                wx.EVT_CHECKBOX, lambda evt, key=field_key: self._on_checkbox_changed(key)
            )
    ...
    def _on_submit(self, event):
        ...
        url = build_affiliate_correction_url(affiliate, corrections, notes, is_affiliated)
    ...
    def _on_copy(self, event):
        ...
        text = copy_affiliate_correction_to_clipboard(affiliate, corrections, notes, is_affiliated)
