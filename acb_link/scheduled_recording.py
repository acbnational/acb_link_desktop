...
class StreamRecorderEngine:
    ...
    def start_recording(self, item: RecordingQueueItem) -> bool:
        ...
        self._recording_thread = threading.Thread(
            target=self._record_thread, args=(item,), daemon=True, name=f"Recording-{item.id}"
        )
...
class ScheduledRecordingManager:
    ...
    def _save(self):
        ...
        try:
            ...
            data = {
                "recordings": [
                    {**asdict(rec), "status": rec.status.value, "recurrence": rec.recurrence.value}
            ...
            # Save presets
            preset_data = {"presets": [asdict(p) for p in self.presets.values()]}
        ...
        for preset_id, name, fmt, bitrate in defaults:
            if preset_id not in self.presets:
                self.presets[preset_id] = RecordingPreset(
                    id=preset_id, name=name, format=fmt, bitrate=bitrate
                )
        self._save()
    ...
    def get_upcoming_recordings(self) -> List[ScheduledRecording]:
        ...
        return sorted(
            [
                r
                for r in self.scheduled_recordings.values()
                if r.status == RecordingStatus.SCHEDULED
                and r.next_run
                and datetime.fromisoformat(r.next_run) > now
            ],
    ...
    def start_scheduler(self):
        ...
        self._scheduler_thread = threading.Thread(
            target=self._scheduler_loop, daemon=True, name="RecordingScheduler"
        )
