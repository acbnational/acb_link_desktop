...
class FeedbackDialog(wx.Dialog):
    ...
    def _create_ui(self):
        ...
        self.feedback_type_choice = wx.Choice(
            panel,
            choices=FeedbackType.all_types()
        )
        ...
        self.feature_area_choice = wx.Choice(
            panel,
            choices=FeatureArea.all_areas()
        )
        ...
        self.description_text = wx.TextCtrl(
            ...
            size=(-1, 180)  # type: ignore[arg-type]
        )
        ...
        self.include_system_info = wx.CheckBox(
            panel,
            label="&Include anonymous system information (helps with bug reports)"
        )
...
def report_bug(parent: Optional[wx.Window] = None, feature_area: str = "") -> int:
    ...
    return show_feedback_dialog(
        ...
        initial_feedback_type=FeedbackType.BUG_REPORT
    )
